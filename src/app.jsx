...
function Dashboard({ owner, setup, setOwner, installPrompt, installApp }) {
  ...
  async function load() {
    try {
      const [e, c, p, a] = await Promise.all([
      ...
      if (e.error || c.error || p.error || a.error) {
        setErr(e.error?.message || c.error?.message || p.error?.message || a.error?.message || 'Unable to load data.');
        return;
      }
    ...
    <>
      ...
      <div className="layout">
        ...
        <main>
          ...
          {tab === 'advances' && (
            <AdvancesView advances={advances} persons={persons} owner={owner} ownerCred={ownerCred} setAuth={setAuth} load={load} setErr={setErr} />
          )}

          {tab === 'summary' && (
            <SummaryView total={total} expenses={expenses} category={category} member={member} monthly={monthly} advances={advances} persons={persons} />
          )}
      ...
      {typeof auth === 'object' && auth?.type === 'advance' && (
        <AdvanceModal advance={auth.advance || null} persons={persons}
          close={() => setAuth(null)} saved={async () => { setAuth(null); await load(); }}
          ownerCred={ownerCred} />
      )}
...
function SummaryView({ total, expenses, category, member, monthly, advances, persons }) {
  return (
    <>
      ...
      <div className="content-grid summary-content">
        ...
        <AdvanceSummaryCard advances={advances} persons={persons} />
      </div>
    </>
  );
}

function AdvancesView({ advances, persons, owner, ownerCred, setAuth, load, setErr }) {
  const totalAdvance = useMemo(() => advances.reduce((s, a) => s + Number(a.amount || 0), 0), [advances]);
  ...
  async function deleteAdvance(id) {
    ...
    try {
      const { data, error } = await supabase.rpc('owner_delete_advance', {
        p_username: ownerCred.username, p_password: ownerCred.password, p_id: id
      });
      if (error || !data?.ok) setErr(error?.message || data?.message || 'Unable to delete advance.');
      else await load();
    } catch (error) { setErr(error?.message || 'Unable to delete advance.'); }
  }

  return (
    <>
      <Head title="Advances" sub="Track money paid in advance by one family member for another or for themselves."
        action={owner && <button className="primary add-button" onClick={() => setAuth('advance')} type="button"><Plus size={17} />Add Advance</button>} />
      ...
      <section className="advance-main-block main-total-block">
        <div className="advance-main-left">
          ...
          <div>
            <span className="advance-main-label">TOTAL ADVANCES</span>
            <strong>{money(totalAdvance)}</strong>
            <p>All advance payments recorded for your home</p>
          </div>
        </div>
        <div className="advance-main-stats">
          <div><b>{advances.length}</b><small>Records</small></div>
          <div><b>{new Set(advances.map(a => String(a.paid_by))).size}</b><small>Payers</small></div>
          <div><b>{new Set(advances.map(a => String(a.paid_for))).size}</b><small>For</small></div>
        </div>
      </section>

      <Card title="Advance history" subtitle="Who paid, who the advance is for, and how much.">
        {!advances.length ? (
          <div className="empty-state-advance">
            <div className="advance-empty-icon"><HandCoins size={24} /></div>
            <b>No advances recorded yet</b>
            <p>Add an advance when any person pays money for another person or for themselves.</p>
            {owner && <button className="primary" onClick={() => setAuth('advance')} type="button"><Plus size={15} />Add First Advance</button>}
          </div>
        ) : (
          <div className="advance-list">
            {advances.map(a => {
              const payer = personMap.get(String(a.paid_by)) || 'Unknown';
              const beneficiary = personMap.get(String(a.paid_for)) || 'Unknown';
              const self = String(a.paid_by) === String(a.paid_for);
              return (
                <article className="advance-card" key={a.id}>
                  <div className="advance-card-icon"><HandCoins size={20} /></div>
                  <div className="advance-card-main">
                    <div className="advance-card-title"><b>{self ? `${payer} — Self Advance` : `${payer} → ${beneficiary}`}</b><span>{dateText(a.advance_date)}</span></div>
                    <div className="advance-card-meta">
                      <span>Paid by <b>{payer}</b></span>
                      <span>{self ? 'For self' : <>For <b>{beneficiary}</b></>}</span>
                      {a.notes && <span>{a.notes}</span>}
                    </div>
                  </div>
                  <strong className="advance-card-amount">{money(a.amount)}</strong>
                  {owner && <div className="advance-card-actions">
                    <button className="icon action-edit" type="button" title="Edit advance" onClick={() => setAuth({ type: 'advance', advance: a })}><Edit3 size={15} /></button>
                    <button className="icon danger" type="button" title="Delete advance" onClick={() => deleteAdvance(a.id)}><Trash2 size={15} /></button>
                  </div>}
                </article>
              );
            })}
...
function AdvanceSummaryCard({ advances, persons }) {
  const personMap = useMemo(() => new Map(persons.map(p => [String(p.id), p.name])), [persons]);
  const rows = useMemo(() => {
    const m = {};
    advances.forEach(a => {
      const id = String(a.paid_for);
      m[id] = (m[id] || 0) + Number(a.amount || 0);
    });
    return Object.entries(m).sort((a, b) => b[1] - a[1]).slice(0, 6);
  }, [advances]);
  const total = advances.reduce((s, a) => s + Number(a.amount || 0), 0);
  return <Card title="Advance overview" subtitle="Total advances recorded for each person"><div className="advance-summary-top"><span><HandCoins size={17} /></span><div><small>TOTAL ADVANCES</small><strong>{money(total)}</strong></div></div>{rows.length ? <div className="advance-summary-list">{rows.map(([id, value]) => { const pct = total ? Math.round(value / total * 100) : 0; return <div className="advance-summary-row" key={id}><div><span className="person-dot">{String(personMap.get(id) || '?').slice(0,1).toUpperCase()}</span><b>{personMap.get(id) || 'Unknown'}</b></div><strong>{money(value)}</strong><em>{pct}%</em><div className="advance-summary-track"><span style={{width:`${Math.max(pct,2)}%`}} /></div></div>; })}</div> : <div className="empty">No advances yet.</div>}</Card>;
}
...
function AdvanceModal({ advance, persons, close, saved, ownerCred }) {
  const [date, setDate] = useState(advance?.advance_date || new Date().toISOString().slice(0, 10));
  const [amount, setAmount] = useState(advance?.amount || '');
  const [paidBy, setPaidBy] = useState(advance?.paid_by || '');
  const [paidFor, setPaidFor] = useState(advance?.paid_for || '');
  const [notes, setNotes] = useState(advance?.notes || '');
  const [self, setSelf] = useState(Boolean(advance && String(advance.paid_by) === String(advance.paid_for)));
  const [busy, setBusy] = useState(false);
  const [err, setErr] = useState('');

  function changePayer(value) {
    setPaidBy(value);
    if (self) setPaidFor(value);
  }
  function toggleSelf(checked) {
    setSelf(checked);
    if (checked) setPaidFor(paidBy);
  }

  async function save(e) {
    e.preventDefault();
    if (!ownerCred) return setErr('Owner session missing. Please login again.');
    if (!paidBy || (!self && !paidFor)) return setErr('Please select who paid and who the advance is for.');
    if (!Number(amount) || Number(amount) <= 0) return setErr('Please enter a valid advance amount.');
    const beneficiary = self ? paidBy : paidFor;
    if (!beneficiary) return setErr('Please select the person the advance is for.');
    setBusy(true); setErr('');
    const args = {
      p_username: ownerCred.username, p_password: ownerCred.password,
      p_advance_date: date, p_amount: Number(amount), p_paid_by: paidBy,
      p_paid_for: beneficiary, p_notes: notes.trim() || null
    };
    try {
      const fn = advance ? 'owner_update_advance' : 'owner_add_advance';
      const { data, error } = await supabase.rpc(fn, advance ? { ...args, p_id: advance.id } : args);
      if (error || !data?.ok) { setErr(error?.message || data?.message || 'Unable to save advance.'); return; }
      await saved();
    } catch (error) { setErr(error?.message || 'Unable to save advance.'); }
    finally { setBusy(false); }
  }

  return <div className="backdrop"><div className="modal advance-modal">
    <div className="modal-head"><div><div className="page-kicker">DREAM HOME</div><h2>{advance ? 'Edit Advance' : 'Add Advance'}</h2><p>Record money paid in advance for another person or yourself.</p></div><button className="icon" onClick={close} type="button" disabled={busy}><X /></button></div>
    <form className="form" onSubmit={save}>
      <div className="grid"><label>Date<input type="date" required value={date} onChange={e => setDate(e.target.value)} disabled={busy} /></label><label>Amount (₹)<input type="number" min="1" step=".01" required value={amount} onChange={e => setAmount(e.target.value)} placeholder="Enter amount" disabled={busy} /></label></div>
      <label>Paid By<select value={paidBy} onChange={e => changePayer(e.target.value)} required disabled={busy}><option value="">Select payer</option>{persons.map(p => <option key={p.id} value={p.id}>{p.name}</option>)}</select></label>
      <label className="self-check"><input type="checkbox" checked={self} onChange={e => toggleSelf(e.target.checked)} disabled={busy || !paidBy} /><span><b>Advance for myself</b><small>Payer and recipient will be the same person.</small></span></label>
      {!self && <label>Advance For<select value={paidFor} onChange={e => setPaidFor(e.target.value)} required disabled={busy}><option value="">Select recipient</option>{persons.map(p => <option key={p.id} value={p.id}>{p.name}</option>)}</select></label>}
      {!persons.length && <div className="notice">Add family members from the Persons tab before recording an advance.</div>}
      <label>Notes<textarea rows="3" value={notes} onChange={e => setNotes(e.target.value)} placeholder="Optional notes" disabled={busy} /></label>
      {err && <div className="notice">{err}</div>}
      <div className="modal-actions"><button type="button" className="secondary" onClick={close} disabled={busy}>Cancel</button><button type="submit" className="primary" disabled={busy || !persons.length}>{busy ? 'Saving…' : <><Check />Save Advance</>}</button></div>
    </form>
  </div></div>;
}
