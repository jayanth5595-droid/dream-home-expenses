...
const categoryIcons = {
  materials: Box, material: Box, cement: Box, bricks: Box,
  labour: HardHat, labor: HardHat, masonry: HardHat,
  payments: CreditCard, payment: CreditCard,
  electrical: Zap, wiring: Zap,
  plumbing: Droplets, pipes: Droplets,
  furniture: Sofa,
  transport: Truck, travel: Car, vehicle: Car,
  food: Utensils, groceries: ShoppingBag, shopping: ShoppingBag,
  household: Home, home: Home, appliances: Box,
  tools: Wrench,
  medical: HeartPulse,
  education: BookOpen,
...
function categoryIconComponent(category) {
  ...
  const match = Object.keys(categoryIcons).find(key => name === key || name.includes(key));
  return categoryIcons[match || 'other'];
}

function CategoryIcon({ category, size = 18, className = '' }) {
              ...
              <tr key={x.id}>
                ...
                <td>
                  <span className="category-chip">
                    <span className="category-emoji"><CategoryIcon category={x.category} size={16} /></span>
        ...
        {rows.map(x => (
          <article className="expense-card" key={x.id}>
            <div className="expense-card-top">
              <div className="expense-card-icon"><CategoryIcon category={x.category} size={18} /></div>
