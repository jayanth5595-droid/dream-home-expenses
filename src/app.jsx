    ...
    <>
      ...
      <div className="layout">
        ...
        <main>
          ...
          {tab === 'dashboard' && (
            <DashboardView total={total} expenses={expenses} category={category} member={member} owner={owner} setAuth={setAuth} />
          )}
...
function DashboardView({ total, expenses, category, member, owner, setAuth }) {
  return (
    <>
      <Head title="Dashboard" sub="A clear view of your home's finances."
        action={owner && <button className="primary add-button" onClick={() => setAuth('expense')} type="button"><Plus size={17} />Add Expense</button>} />

      <section className="hero-card apple-hero">
        <div className="hero-copy">
          <span className="eyebrow">DREAM HOME · OVERVIEW</span>
          <h2>{money(total)}</h2>
          <p>Total invested so far</p>
          <div className="hero-meta">
            <span><Receipt size={14} />{expenses.length} expenses</span>
            <span><Wallet size={14} />{category.length} categories</span>
          </div>
        </div>
        <div className="hero-architecture" aria-hidden="true">
          <div className="hero-house"><House size={76} strokeWidth={1.15} /></div>
